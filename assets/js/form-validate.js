	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#GE- Contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#GE- Contact').find('input,textarea').attr({value:''});
				jQuery('#GE- Contact').css({opacity:1});
				jQuery('#GE- Contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#GE- Contact').animate({opacity:0.3});
		jQuery('#GE- Contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#GE- Contact').find('input,textarea').css('border','none');
